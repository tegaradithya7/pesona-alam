		...
		**/
			...
			$('input.form-control ',wrapper).on('change', function(e){
				...
				$.ajax({
					...
					success: function(xhr){
						_files.val('');
						if(xhr.status=='success'){
							$('[type="hidden"]',wrapper).after('<div class="fileUploadBlock"><i class="fa fa-upload"></i>&nbsp;<a href="'+xhr.upload_path+'" target="blank">'+xhr.file_name+'</a><div class="deleteFile"><i class="fa fa-trash"></i></div></div>');
							$('.deleteFile', wrapper).on('click',function(e){
								$('.form-control',wrapper).show();
								$('input[type=hidden]',wrapper).val('');
								$('.fileUploadBlock', wrapper).remove();
		...
		generateMultipleFile: function(key, data, type){
			...
			if(data.value!=''){
				...
				$.each(content, function(key, value){
					...
					$(wrapper).append('<div class="fileUploadBlock" id="'+data.field+'['+(key+1)+']"><i class="fa fa-upload"></i>&nbsp;<a href="'+data.upload_path+'/'+value+'" target="_blank">'+value+'</a><div class="deleteFile"><i class="fa fa-trash"></i></div></div>');
					$('.deleteFile', wrapper).on('click',function(e){
						$('.form-control',wrapper).show();
						$('input[type=hidden]',wrapper).val('');
						$('.fileUploadBlock', wrapper).remove();
			...
			function process(obj){
				...
				$.ajax({
					...
					success: function(xhr){
						_files.val('');
						if(xhr.status=='success'){
							$(_files).after('<div class="fileUploadBlock" id="'+data.field+'_'+no+'" no="'+no+'"><i class="fa fa-upload"></i>&nbsp;<a href="'+xhr.upload_path+'" target="blank">'+xhr.file_name+'</a><div class="deleteFile" ><i class="fa fa-trash"></i></div></div>');
							$('.deleteFile', wrapper).on('click',function(e){
		...
		generateReadonly: function(key, data, type){
			...
			if(type=='file'){
				...
				var content = data.value.split(',');
				$.each(content, function(key, value){
					$('.multipleLampiran', wrapper).append('<p><a href="'+base_url+'assets/lampiran/'+data.field+'/'+value+'" target="blank">'+value+'</a></p>')
				});
			}else if(type=='radio'){
		...
		generateCheckbox: function(key, data){
			...
			$.each(data.source, function(keys, value){
				...
				_check += "<input type='checkbox' value='"+keys+"' "+_checked+" name='"+data.field+"[]' class='form-control "+data.class+"'><label>"+value+"</label> ";
