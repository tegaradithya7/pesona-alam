...
class Utility{
	...
	function generateLinkTable($key, $value,$sort){
		return '<a href="?'.$this->generateLink('sort','desc').'&sort='.(($sort[$key] == 'asc') ? 'desc' : 'asc') .'&by='.$key.'">'.$value.'<i class="fa fa-sort-'.(($sort[$key] == 'asc') ? 'desc' : 'asc').'"></i></a>';
	}
	...
	function mail($to,$message,$subject){
		...
		$this->CI->email->from('vms@pgn.co.id', 'VMS PGN');
		$this->CI->email->to($to); 
	...
	function set_admin_note($id_role,$id_sbu,$message){
			$this->CI->load->library('utility');
			$this->CI->db->insert('tr_note',array(
												'value'			=>$message,
												'id_role'		=>$id_role,
												'id_sbu'		=>$id_sbu,
			...
			$admin_data = "SELECT * FROM ms_admin WHERE id_role = ? AND id_sbu = ?";
			$admin_data = $this->CI->db->query($admin_data,array($id_role,$id_sbu))->result_array();
			foreach ($admin_data as $key => $value) {
				$this->mail($admin_data['email'],'Notifikasi verifikasi berkas Sistem Manajemen Penyedia Barang & Jasa PT Perusahaan Gas Negara',$message);
			}
			
	}
	function set_notification_admin($id_sbu, $id_role, $message){
		$this->CI->db->insert('tr_note',array(
											'value'			=>$message,
											'id_role'		=>$id_role,
											'id_sbu'		=>$id_sbu,
	...
	function set_notification_vendor($id_vendor,$message){
		$this->CI->db->insert('tr_note',array(
												'value'			=>$message,
												'id_vendor'		=>$id_vendor,
												'entry_stamp'	=>date('Y-m-d H:i:s'),
	...
	function send_notification_vendor($id_vendor,$subject, $message){
		$user = "SELECT * FROM ms_vendor_admistrasi WHERE id_vendor = ?";
		$user = $this->CI->db->query($user,array($id_vendor))->row_array();
		$this->CI->utility->mail($user['vendor_email'],$subject,$message);
	}

	function send_notification_admin($id_admin,$subject, $message){
		$admin_data = "SELECT * FROM ms_admin WHERE id = ?";
		$admin_data = $this->CI->db->query($admin_data,array($id_vendor))->row_array();
		$this->CI->utility->mail($admin_data['email'],$subject,$message);
	}
	
	function is_konstruksi($id_vendor){
		$this->CI->load->model('k3/k3_model','k3');
		return $this->CI->k3->is_konstruksi($id_vendor);
	}	
}