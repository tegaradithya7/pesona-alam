...
$(function(){
	...
	var table = $('#tableGenerator').tableGenerator({
		url: '<?php echo site_url('master/dining/getData/'.$id); ?>',
		data: dataPost,
		headers: [
			{
				"key"	: "name",
		...
		},
		{
			renderCell: function(data, row, key, el){
				var html = '';
				html +=editButton(site_url+"master/dining/form/edit/"+data[2].value, data[2].value);
				html +=deleteButton(site_url+"master/dining/remove/"+data[2].value, data[2].value);
				return html;
			},
			target : [3]
		...
		additionFeature: function(el){
			el.append(insertButton(site_url+"master/dining/form/add/<?php echo $id;?>"));
		},
		finish: function(){
